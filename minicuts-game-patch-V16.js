(function(){
  ...
  function safe(fn){try{fn();}catch(e){console.warn('V16 roadmap patch:',e.message);}}
  ...
  function miniBar(points){
    ...
    return '<div class="v16-mini-road"><div class="v16-mini-road-fill" style="width:'+pct+'%"></div><div class="v16-mini-road-pin" style="left:'+pct+'%"></div></div><div class="v16-mini-txt">L1 → L6 · '+i.pts+' pts'+(i.next?' · '+i.needed+' pts to L'+i.next.level:' · Max')+'</div>';
  }
  ...
  function cleanupOldLevelSystems(){
    try{
      var tier=el('tierList');if(tier)tier.innerHTML='';
      var mt=el('milestoneTrack');if(mt)mt.style.display='none';
      var cards=document.querySelectorAll('[id*="tier"], [class*="tier"], [id*="milestone"], [class*="milestone"]');
      for(var i=0;i<cards.length;i++){if(cards[i].id!=='v16Roadmap'&&!cards[i].closest('#v16Roadmap'))cards[i].style.display='none';}
    }catch(e){}
  }

  function addLeftGuide(){
    var left=document.querySelector('.left-panel');if(!left)return;
    var old=el('v16LevelGuide');if(old)old.remove();
    var rows=LEVELS.map(function(l){return '<div class="v16-level-row"><div class="v16-level-icon">'+l.icon+'</div><div><div class="v16-level-title">L'+l.level+' · '+l.badge+' · '+l.pts+' pts</div><div class="v16-level-desc">Prize: Mini Surprise Gift</div></div></div>';}).join('');
  ...
  function patchProfile(){
    ...
    var oldRender=window.renderProfile;if(typeof oldRender==='function')window.renderProfile=function(){oldRender();setTimeout(function(){cleanupOldLevelSystems();addRoadmap();addLeftGuide();},80);};
    var oldGo=window.goProfile;if(typeof oldGo==='function')window.goProfile=function(){oldGo();setTimeout(function(){cleanupOldLevelSystems();addRoadmap();addLeftGuide();},100);};
